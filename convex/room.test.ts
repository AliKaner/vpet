...
test("home upgrades charge once, are shared, expand garden capacity and stop at max level",async()=>{
  ...
  const [ownerId,partnerId]=await t.run(async ctx=>{
    const owner=await ctx.db.insert("users",{name:"Owner",coins:1000});
  ...
  expect((await owner.query(api.users.getMyProfile)).coins).toBe(920);
  ...
  expect((await owner.query(api.users.getMyProfile)).coins).toBe(390);
  await expect(owner.mutation(api.decor.upgradeHome,{expectedLevel:4})).rejects.toThrow();
  expect((await partner.query(api.decor.getMyRoom)).level).toBe(4);
});
