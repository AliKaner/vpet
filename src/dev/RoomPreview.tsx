...
import { BASE_TILES } from "../../convex/lib/roomTiles";
...
export function Preview() {
  ...
  return <main style={{ maxWidth:760,margin:"24px auto",padding:16 }}><ThemeFilter value={theme} onChange={chooseTheme} /><RoomSceneView room={room} character={character} coins={1000000} onBuyTile={async tile=>setRoom(prev=>({...prev,tiles:[...(prev.tiles??BASE_TILES),tile],tilePurchases:(prev.tilePurchases??0)+1}))} onUpgrade={async()=>setRoom(prev=>({...prev,level:(prev.level??1)+1}))} picker={close=><div className="preview-picker-grid">{SHOP_CATALOG.filter(i=>i.kind==="furniture").map(item=><button key={item.id} onClick={()=>{setRoom(prev=>({...prev,placements:[...prev.placements.filter(p=>p.itemId!==item.id),{itemId:item.id,x:30,y:30,area:item.kind==="furniture"&&item.outdoor?"garden":"room",flipped:false}]}));close();}}><FurnitureArt id={item.id}/>{item.label}</button>)}</div>} move={async args => { setRoom(prev => ({ ...prev,placements:prev.placements.map(p => p.itemId === args.itemId ? {...p,...args} : p) })); }} remove={async args => { setRoom(prev => ({ ...prev,placements:prev.placements.filter(p => p.itemId !== args.itemId) })); }}>
