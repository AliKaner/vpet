...
export function TileSurfaces({tiles,wallpaperId,floorId}:{tiles:RoomTile[];wallpaperId?:string;floorId?:string}) {
  const g=roomGeometry(tiles),occupied=new Set(tiles.map(tileKey));
  ...
  return <svg className="room-shell" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
    ...
    {tiles.map(({x,y})=><polygon key={`floor${x},${y}`} points={[point(x,y),point(x+1,y),point(x+1,y+1),point(x,y+1)].join(" ")} fill={floorColor} stroke="#a5846333" strokeWidth=".25"/>)}
